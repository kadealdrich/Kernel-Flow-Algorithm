...
value_and_grad = jit(jax.value_and_grad(pen_crit_sigma_jit, argnums=0))
KRR_lapL1_jit = jit(KRR_lapL1)



# function for getting gradient step using rho criterion penalized by mse
# updates both sigma and lambda at each iteration
@partial(jit,)
def make_gd_step_reg_pen_crit(params, x_tr, x_val, y_tr, y_val, x_fine, x_coarse, y_fine, y_coarse, mse_weight, step_size):
        
    crit, grad = value_and_grad(params, x_fine, x_coarse, y_fine, y_coarse, x_tr, x_val, y_tr, y_val, mse_weight)
    
    # step size is determined before calling this function
    params = params - step_size*grad

    return params, crit


# function for making gd step only updating sigma for Laplacian kernel
@jit
def make_gd_step_sigma(sigma, x_fine, x_coarse, y_fine, y_coarse, x_tr, x_val, y_tr, y_val, mse_weight, step_size):
        
    crit, grad = value_and_grad(sigma, x_fine, x_coarse, y_fine, y_coarse, x_tr, x_val, y_tr, y_val, mse_weight)

    # step size determined before calling this function
    new_sigma = sigma - step_size*grad

    return new_sigma, crit
...
# gradient descent on kernel parameter sigma and regularization parameter lambda
def run_gd(max_iter, params_init, key, mse_weight, split_thresh = 1):
    ...
    init_crit = calc_pen_crit_lapL1(params= params_init, x_fine=x_fine, x_coarse=x_coarse, y_fine = y_tr, y_coarse = y_coarse, x_tr = x_tr, x_val = x_val, y_tr = y_tr, y_val = y_val)
    ...
    # run descent over max_iter iterations
        ...
        params, crit = make_gd_step_reg_pen_crit(params, x_tr, x_val, y_tr, y_val, x_fine, x_coarse, y_fine, y_coarse, mse_weight, step_now)
    ...
    return pd.DataFrame({
        ...
        "sigma": traj[:,1], # column for the sigma value at each iteration
...
def run_gd_sigma(max_iter, sigma_init, key, mse_weight=0.5, split_thresh=1):
    ...
    # initial loss
    loss_trace[0] = pen_crit_sigma_jit(sigma,
    ...
    for i in range(1, max_iter + 1):
        ...
        sigma, loss = make_gd_step_sigma(sigma,
                                    x_fine, x_coarse, y_fine, y_coarse,
                                    x_tr,   x_val,    y_tr,   y_val,
                                    mse_weight, step)
        traj[i]       = sigma
...
def plot_run_gd_sigma(
    ...
    *,
    ...
    """
    Visualise the optimisation trace produced by `run_gd_sigma`.

    Parameters
    ----------
    df : pandas.DataFrame  (columns MUST include 'iteration', 'sigma'.
    ...
    eps_zero : float       |sigma| < eps_zero → treat as “≈ 0”.
    ...
    ax.plot(iters, sigma, label="sigma")

    # mark first NaN or near-zero γ
    bad_idx = np.where(np.isnan(sigma) | (np.abs(sigma) < eps_zero))[0]
    if bad_idx.size:
        ax.axvline(iters[bad_idx[0]], color="red", lw=2,
                   label="sigma ≈ 0 or NaN")
    ...
    ax.set_ylabel("sigma")
    ax.set_title("Trace of sigma over iterations")
   ...
   # run KRR (make sure to pass your train/test sets correctly)
    ...
    plt.legend(title=f'sigma = {sigma_gd:.3f}, λ = {lam_fixed:.3f}, mse = {final_mse:.5f})') 
    plt.tight_layout()
    plt.show()
    plt.show()
...
def plot_sigma_lambda_results(df):
    ...
    plt.legend(title=f'initial sigma = {sigma_init:.1f}, initial λ = {lam_init:.1f}, split threshold = 1000') 
    ...
    y_pred = jnp.asarray(KRR_lapL1_jit(w = sigma_gd, lam = lam_gd, x_train = x_train_first, y_train = y_train_first, x_test = x_test_first)) # converting to numpy array
    ...
    plt.ylabel('y')
    plt.title('True Y vs KRR prediction on Test Set using L1 Laplacian Kernel')
    plt.legend(title=f'sigma = {sigma_gd:.3f}, λ = {lam_gd:.3f}, mse = {final_mse:.5f})') 
    plt.tight_layout()
    plt.show()


print("...Gradient descent wrt sigma...")
# testing gradient descent wrt sigma 
df1 = run_gd_sigma(
    max_iter = 1, 
    sigma_init = jnp.asarray(sigma_init, dtype=jnp.float32),
    key = key_init,
    mse_weight = 1, 
